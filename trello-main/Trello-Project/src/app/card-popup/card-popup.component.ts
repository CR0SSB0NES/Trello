...
export class CoverComponent {
  ...
  @Output() closeCoverBottom = new EventEmitter<boolean>();
  @Input() board_info: any
  card: any
  bgkColor: any = "#d7d7d7";
  styleColor: any = "#b1b1b1"

  constructor(public boardList:BoardlistsService, private elm: ElementRef) {
  }

  ngOnInit(){
    this.card = this.board_info[2];
    if(this.card.cardDetails.cover && this.card.cardDetails.cover.length){
      this.styleColor = "#fff";
      this.bgkColor = this.card.cardDetails.cover[0];
      if(this.card.cardDetails.cover[1] == 1)
        document.querySelector(".cover-style1")?.classList.add("selected-cover");
      else
        document.querySelector(".cover-style2")?.classList.add("selected-cover");
    }
  ...
  closeCoverPopup(e: Event) {
    this.closeCoverBottom.emit()
  }
  ...
  setCover(color: any) {
    if(this.card.cardDetails.cover.length)
      this.card.cardDetails.cover.splice(0,1);
    if(this.card.cardDetails.cover[1] == null){
      this.card.cardDetails.cover[1] = 1;
      document.querySelector(".cover-style1")?.classList.add("selected-cover");
    }
    this.card.cardDetails.cover[0] = color;
    console.log('Card: ', this.card.cardDetails);
    this.bgkColor = this.card.cardDetails.cover[0];
    this.styleColor = "#fff"
    this.boardList.storeData();
  }

  removeCover(cover1?: any, cover2?: any) {
    this.card.cardDetails.cover.splice(0,2);
    this.bgkColor = "#d7d7d7";
    this.styleColor = "#b1b1b1";
    if(cover1 && cover2) {
      cover1.classList.remove("selected-cover");
      cover2.classList.remove("selected-cover");
    }
    this.boardList.storeData();
  }

  updateStyle(value: any, target :any, previous: any) {
    if(this.card.cardDetails.cover.length >= 1){
      target.classList.add("selected-cover");
      if(previous.classList.contains("selected-cover"))
        previous.classList.remove("selected-cover");
      this.card.cardDetails.cover[1] = value;
      console.log(this.card.cardDetails.cover);
      this.boardList.storeData();
    }
  }
