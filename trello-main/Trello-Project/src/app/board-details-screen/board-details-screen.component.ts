...
export class BoardDetailsScreenComponent implements OnInit {
  ...
  constructor(public boardList: BoardlistsService, public activeRouter: ActivatedRoute, private location:Location, public _router: Router) {
  ...
  let a = this.activeRouter.pathFromRoot
      console.log(a[0]);
      console.log(a[0].snapshot);
  ...
  currentBoard(id: number) {
    ...
    this.boardList.findBoards(id).subscribe(x => parcel.push(x));
    // this.boardList.findBoards(id).subscribe({next(x){(x:any) => {parcel.push(x)}}});
    this.chosenBoard = parcel[0]
    // console.log('************');
    // console.log(parcel);
    // console.log('************');
  ...
  addCard(cardsList: any) {
    ...
    if (value) {
      if (!cardsList.cards) {
        ...
        cardsList.cards.push(cardsList = {
          cardTitle: value,
          cardDetails: cardDetailsStr
        })
    }
